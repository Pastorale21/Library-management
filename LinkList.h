...
//成员函数声明
	...
	T DeleteCurNode() ;//noexcept ;//throw;				// 删除当前结点，其下一个结点（成为新的当前结点），值返回被删结点数据域数值
	T ModifyData(const T &t) ;//noexcept;//throw	;	// 修改(替换)当前结点的数据域数值

	int NumNodes() const;						// 返回结点数
	int CurPos() const;							// 返回当前结点的序号（-1表示无当前结点）
	Node<T> *CurNode();							// 返回当前结点的地址（NULL表示无当前结点）
	T & CurData();// noexcept;// throw					// 引用返回当前结点数据域成员，函数调用表达式可做左值

	Node<T> *GoTop(), *Go(int n), *GoBottom();	// 当前结点绝对定位：首结点、第n个结点(n从0起)、尾结点
	Node<T> *Skip(int n=1);// noexcept ;//throw		// 当前结点相对定位（n可为负数），默认n为1表示下一个结点
	template <typename TYPE> Node<T> *Locate(const TYPE &x, bool newsearch=false); // 当前结点依条件（给定TYPE类型）定位
	template <typename TYPE>
    Node<T>* Locate(bool (*func)(TYPE, std::string), std::string condition, bool newsearch);
	template <typename TYPE> void Sort(const TYPE &x, bool ascending=true);	// 根据TYPE类型排序（升序或降序）
	void Reverse();								// 链表结点倒置
	T* findByTitle(const string &title); 
	T* findUser(const string &ID, const string &name);
	void findUnreturnedBooks();
	int displayUnreturnedBooks(vector<Node<T>*>& unreturnedBooks);
	void findUnreturnedBooks(LinkList<string> &unreturnedbooks);
	bool isEmpty() const;
...
template <typename T>
T LinkList<T>::DeleteCurNode() 	//noexcept	// 删除当前结点，其下一个结点（成为新的当前结点），值返回被删结点数据域数值
{
	if(cur_node==NULL || head==NULL)
		throw -1;
	Node<T> *p;
...
template <typename T>
T LinkList<T>::ModifyData(const T &t) //noexcept // 修改(替换)当前结点的数据域数值
{
	if(cur_node==NULL)
		throw -1;
...
template <typename T>
T & LinkList<T>::CurData() 	//noexcept		// 引用返回当前结点数据域成员，函数调用表达式可做左值
{
	if(cur_node!=NULL)
...
template <typename T>
Node<T> *LinkList<T>::Skip(int n) //noexcept	// 当前结点相对定位（n可为负数），默认n为1表示下一个结点
{
	if(cur_node == NULL)
		throw -1;
	int m = CurPos() + n;
...
template <typename T> template <typename TYPE>
Node<T> *LinkList<T>::Locate(bool (*func)(TYPE,string),string condition, bool newsearch) // 当前结点依条件（给定TYPE类型）定位
{
    static Node<T> *p=head;
    if(newsearch)
    {
    ...
    for(; p!=NULL && !func(p->data,condition); p=p->next)  // for循环的表达式1为空：从链表的"当前"结点起继续查找
        ;                                      // TYPE()为类型强制转换运算符
    cur_node = p;
    if(p!=NULL)       // 如果找到，在返回找到的结点地址（cur_node）之前，p"悄悄"地向前走一步，为继续查找做好准备
        p = p->next;
...
template <typename T>
T* LinkList<T>::findByTitle(const string &title) {
    Node<T>* current = head;
    while (current != nullptr) {
        if (current->data.getTitle() == title) {
			cur_node = current;
            return &current->data;  // 返回找到的书籍的地址
        }
...
template <typename T>
T* LinkList<T>::findUser(const string &ID, const string &name) {
    Node<T>* current = head;
    while (current != nullptr) {
        if (current->data.getID() == ID && current->data.getName() == name) {
            cur_node = current;
            return &current->data;  // 返回找到的用户数据的地址
        }
        current = current->next;
    }
    return nullptr;  // 未找到
...
template <typename T>
void LinkList<T>::findUnreturnedBooks() {
    Node<T>* current = head;
    bool hasUnreturned = false;  // 标志是否有未归还的书籍

    while (current != nullptr) {
        if (current->data.getReturndate().empty()) {  // 检查 returnDate 是否为空，表示未归还
            cout << current->data << endl;  // 输出未归还的书籍记录
            hasUnreturned = true;
        }
        current = current->next;
    }
...
template <typename T>
int LinkList<T>::displayUnreturnedBooks(vector<Node<T>*>& unreturnedBooks) {
    Node<T>* current = head;
    int index = 0;
    bool hasUnreturned = false;  // 标志是否有未归还的书籍
    ...
    while (current != nullptr) {
        if (current->data.getReturndate().empty()) {  // 检查 returnDate 是否为空，表示未归还
            unreturnedBooks.push_back(current);  // 保存未归还的书籍节点指针
            cout << index + 1 << ". " << current->data << endl;  // 输出书籍编号及其信息
            hasUnreturned = true;
            index++;
        }
        current = current->next;
    ...
    return index;  // 返回未归还书籍的总数
}
template <typename T>
void LinkList<T>::findUnreturnedBooks(LinkList<string> &unreturnedbooks) {
    Node<T>* current = head;
    while (current != nullptr) {
        if (current->data.getReturndate().empty()) {  // 找到尚未归还的书
            unreturnedbooks.Append(current->data.getBookName());  // 添加到未归还书籍链表
        }
        current = current->next;
    }
}
...
template <typename T>
bool LinkList<T>::contains(const T &data) const {
    Node<T>* current = head;
    while (current != nullptr) {
        if (current->data == data) {  // 如果找到匹配的数据，返回 true
            return true;
        }
        current = current->next;
    }
